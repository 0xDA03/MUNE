...
from Export import generateDAT, generateTXT, generatePlot
...
def main():
    ...
    RE_STRENGTHS = [0.2, 0.6, 1]                # strength of reinnervation represents the fraction of degenerated nerves that are to be compensated for
            ...
            for re_strength in RE_STRENGTHS:
                for i in range(len(SEEDS)):
                    ...
                    while len(mu_sizes) >= 5:
                        ...
                        mu_sizes = degenerate(mu_sizes, mu_count, de_method, re_method, re_strength, rng)      # handle degeneration and reinnervation of motor units
                    
                    re_str = re_strength*100 if re_method != "none" else "0.0"                                                   # console message to indicate reinnervation strength
                    print_progress(i+1, len(SEEDS), f"Running '{de_method}' deinnervation and {re_str}% '{re_method}' reinnervation", '', 0, 50)     # display a progress bar for visual feedback
                
                if re_method == "none":
                    break   # break unnecessary loop of varying reinnervation strengths when there is no compensation 


def degenerate(mu_sizes, mu_count, de_method, re_method, re_strength, rng):
    ...
    if de_method == "random":
        ...
        de_sizes = mu_sizes[int(mu_count*0.5):]     # deinnervate the largest motor units
    
    mu_sizes = np.setdiff1d(mu_sizes, de_sizes, assume_unique=True)     # remove these degenerated units from the modeled units
   
    if re_method == "random":
        re_sizes = rng.choice(de_sizes, size=int(mu_count*re_strength*0.5), replace=False)                          # randomly select X% of the degenerated units to reinnervate
        re_indices = rng.choice(list(range(len(mu_sizes))), size=int(mu_count*re_strength*0.5), replace=False)      # randomly select X% of the remaining units to compensate
